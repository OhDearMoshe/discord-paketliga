...
class PointDaoTest {
    ...
    fun canSuccessfullyInsertWinIntoTable() {
        ...
        val result = testWrapper.executeSimpleQuery<Point>("""SELECT * FROM POINT""".trimIndent())
    ...
    fun canSuccessfullyUpdateWinOnConflictIntoTable() {
        ...
        val firstResult = testWrapper.executeSimpleQuery<Point>("""SELECT * FROM POINT""".trimIndent())
        ...
        target.addWin(expected)
        val secondResult = testWrapper.executeSimpleQuery<Point>("""SELECT * FROM POINT""".trimIndent())
    ...
    private fun createdPoint(): Point {
        return Point(pointId = 1, userId = "Z", played = 1, won = 1, lost = 1, drawn = 0, bonus = 0, totalPoint = 1F)
    }
}
