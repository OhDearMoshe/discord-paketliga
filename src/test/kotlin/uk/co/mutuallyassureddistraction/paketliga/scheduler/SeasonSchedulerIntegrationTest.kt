...
class SeasonSchedulerIntegrationTest {
    ...
    fun setUp() {
        val dynamicTime =
            LocalDateTime.now().plusSeconds(30).format(DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss"))
    ...
    fun `should migrate schema and execute job`() {
        ...
        await().atMost(60, TimeUnit.SECONDS).untilAsserted { verify(exactly = 1) { gameDao.findActiveGameById(1) } }
    }
}
