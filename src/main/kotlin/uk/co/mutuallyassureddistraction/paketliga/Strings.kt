...
const val DeliveryTimeOutsideWindowGameVoidMessage = "Delivery time is outside of delivery window game void"
...
fun gameAnnouncementMessage(
    ...
    ":postal_horn: ${gameNameStringMaker(gameName, member, username, gameId)} | package is arriving between " +
        "${guessWindow.startAsHumanFriendlyString()} and " +
        "${guessWindow.endAsHumanFriendlyString()}. " +
        "Guesses accepted until ${guessWindow.deadlineAsHumanFriendlyString()}"

fun gameUpdateMessage(gameId: Int, guessWindow: GuessWindow) =
